...
void test_extra()
{
    ...
    try {
        list.removeBetween(it, it1);
        assert(false);
    }
    catch (std::exception &ex) {
        assert(true);
    }
    list.removeBetween(it1, it);
//    it1.current = -1;
//    try {
//        it1.current = -1;
//        assert(false);
...
void testQuantity() {
	...
	testIteratorSteps(l1);
	//something is VERY wrong with the line below - it literally goes to infinity (or I don't have the patience to wait)
	// you should look into why it's happening althought I'm not really sure
	for (int i = 0; i < 20000; i = i + 5) {
		l1.addToEnd(i);
	}

    // and interestingly enough, this crashes this as well if you uncomment both at the same time
	testIteratorSteps(l1);
	IteratedList l2;
	pos = 1;
